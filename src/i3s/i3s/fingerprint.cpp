...
void FingerPrint::toArray(double *a)
{
	for(int i=0; i<cnt; i++)
	{
		a[i*9  ] = elt[i].data[0].getX();
		a[i*9+1] = elt[i].data[0].getY();
		a[i*9+2] = elt[i].data[1].getX();
		a[i*9+3] = elt[i].data[1].getY();
		a[i*9+4] = elt[i].data[2].getX();
		a[i*9+5] = elt[i].data[2].getY();
		a[i*9+6] = elt[i].data[3].getX();
		a[i*9+7] = elt[i].data[3].getY();
	}
...
/***
	...
	depends largely on the size of the image as we calculate in image coordinates
***/
