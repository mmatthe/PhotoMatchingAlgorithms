...
double Element::calcSimilarityRate(const Element& e) const
{
	...
	if(I3S_type == 2 || I3S_type == 3)	// PATTERN
		return rate;
	...
	if(e.angle > -1 && angle > -1) {
		double diff = fabsf(e.angle - angle);
...
bool Element::matches(const Element& e) const
{
	...
	if(I3S_type == 3) {					// PATTERN+ only
		if(withinRange(fgratio, e.fgratio, maxFillDiff) == false)
			return false;
	}
	if(I3S_type == 2 || I3S_type == 3)	// PATTERN 
		return true;
