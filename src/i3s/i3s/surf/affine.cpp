...
void gaussj(float a[3][3], int n, float b[3][1], int m);



void doAffine(float x, float y, float matrix[6], float& outx, float& outy)
{
	outx = x*matrix[0] + y*matrix[1] + matrix[2];
	outy = x*matrix[3] + y*matrix[4] + matrix[5];

}

cv::Point2f doAffine(float x, float y, float matrix[6]) {
	float outx, outy;
	doAffine(x, y, matrix, outx, outy);
	return cv::Point2f(outx, outy);
}

void calcAffine
  (float from1x, float from1y, float from2x, float from2y, float from3x, float from3y,
   float to1x, float to1y, float to2x, float to2y, float to3x, float to3y, 
   float *matrix)
{
   float a1[3][3];
   float b1[3][1];
